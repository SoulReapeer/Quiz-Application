...
// Function prototypes
void saveScore(const PlayerScore& player);
...
int main() {
    ...
    string questions[100] = {
        ...
        "What is the integral of sin(x) with respect to xx?",///37
    ...
    string options[100][4] = {
        /*1*/{"1) 2xex22xe^{x^2}","2) ex2e^{x^2}","3) x2ex2x^2e^{x^2}","4) xexx e^x"},
        ...
        /*37*/{ "1) cos(x)","2) -cos(x)","3) -sin(x)","4) tan(x)"},
        ...
        /*98*/{ "1) Non-random mating","2) Natural selection","3) Large population size","4) Mutation occurring frequently"},
    ...
    int correctAnswers[100] = {1,3,3,2,3,3,4,2,1,1,
                               4,3,3,1,2,2,3,2,2,1,
                               2,2,3,1,2,2,2,4,4,3,
                               3,2,1,2,2,3,2,2,1,3,
                               2,2,2,2,2,1,2,2,2,2,
    ...
    if (menuChoice == 1) {
        ...
        if (playOption == 'y' || playOption == 'Y') {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                cout << "\nQuestion " << i + 1 << ": " << questions[questionIndex] << endl;
                for (int j = 0; j < 4; j++) {
                ...
                cout << "Time taken for this question: " << elapsed << " seconds" << endl;
            }

            auto totalEnd = chrono::steady_clock::now();
            auto totalTime = chrono::duration_cast<chrono::seconds>(totalEnd - totalStart).count();

            cout << "\nQuiz over! " << playerName << ", you scored " << score << " out of 10." << endl;
            ...
            currentPlayer.date = getCurrentDateTime();
...
void saveScore(const PlayerScore& player) {
    ...
    if (file.is_open()) {
        file << player.name << "," << player.score << "," << player.totalTime << "," << player.date << endl;
        file.close();
    } else {
...
void displayHighScores() {
    ...
    if (file.is_open()) {
        ...
        for (const auto& score : scores) {
            if (count++ >= 10) break; // Show top 10 scores
            cout << setw(20) << left << score.name
                 << setw(10) << score.score
                 << setw(15) << score.totalTime
                 << setw(25) << score.date << endl;
        }
